...
export default function StudentAdmissionPage() {
  ...
  const handleSearch = () => {
    // The useEffect above handles filtering, this function is just to trigger it if needed
    // In this setup, filtering is instant so a search button is almost redundant, but good for UX.
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-6xl">
                            ...
                            <Popover>
                                <PopoverTrigger asChild>
                                    <Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !formData.admissionDate && "text-muted-foreground")}>
                                        ...
                                        {formData.admissionDate ? format(formData.admissionDate, "PPP") : <span>Pick a date</span>}
                                    </Button>
                                </PopoverTrigger>
                ...
                <DialogFooter className="pt-6">
                    <Button onClick={handleSubmit} disabled={isLoading}>{isLoading ? "Submitting..." : "Submit Now"}</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    );
