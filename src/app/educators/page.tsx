      ...
      <main className="flex-1">
        <section className="container mx-auto px-4 py-20 md:py-28">
          <div className="text-center mb-16">
            <h1 className="text-4xl md:text-5xl font-bold text-primary tracking-tight">Our Educators</h1>
              ...
              {educators.map((educator) => (
                <Card key={educator.id} className="text-center overflow-hidden shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group border-border/50 rounded-xl">
                    <CardContent className="p-8">
                        <Avatar className="w-36 h-36 mx-auto mb-6 border-4 border-primary/10 group-hover:border-accent transition-colors duration-300">
                            <AvatarImage src={educator.imageUrl || "https://picsum.photos/seed/educator/200"} alt={educator.name} className="object-cover" />
                            <AvatarFallback className="text-3xl">{educator.name.split(' ').map(n => n[0]).join('')}</AvatarFallback>
                        </Avatar>
                        <h3 className="text-xl font-bold text-primary">{educator.name}</h3>
                        <p className="text-accent font-medium">{educator.title}</p>
                    </CardContent>
                </Card>
              ))}
            </div>
