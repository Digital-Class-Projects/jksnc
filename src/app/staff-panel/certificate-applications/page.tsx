...
export default function StaffCertificateApplicationsPage() {
  ...
  useEffect(() => {
    ...
    const fetchApplications = async () => {
        ...
        try {
            ...
            const staffSnapshot = await get(staffQuery);

            if (!staffSnapshot.exists()) {
                toast({ variant: 'destructive', title: 'Staff Not Found', description: "Could not identify your staff profile." });
                setIsLoading(false);
                return;
            }
            ...
            if (!staffBranchId) {
                toast({ variant: 'destructive', title: 'No Branch Assigned', description: "Your staff profile is not assigned to a branch." });
                setIsLoading(false);
                return;
            }
              ...
              if (snapshot.exists()) {
                snapshot.forEach((studentSnapshot) => {
                  ...
                  studentSnapshot.forEach((appSnapshot) => {
                    ...
                    if (appData.branchId === staffBranchId) {
                      allApps.push({
    ...
    <div className="p-4 md:p-8">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Review and manage certificate applications for your assigned branch.
          </CardDescription>
        </CardHeader>
        <CardContent>
            <Table>
